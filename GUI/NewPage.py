...
from PyQt6.QtGui import QPalette  # Import QPalette

# --- Example map info data filename ---
MAP_INFO_FILE = "map_info.json"
# --- Example state machine data filename ---
...
class StateMachineEditorPage(QWidget):
    def __init__(self, back_callback):
        ...
        self.map_info_data = {}  # Store data read from map_info.json
        self.config = []  # Store config info ["Name of pkg", "Author"]
        self.states = []  # Store state machine data
        self.currently_selected_state_index = -1  # Currently selected state index
        self.search_results = []  # Store search results

        self.init_ui()
        self.load_map_info()  # Load map info on initialization
    ...
    def init_ui(self):
        ...
        # --- 2.2 Right: State Details Editor ---
        ...
        self.key_input.setReadOnly(True)  # Key is generated and managed by system, users should not edit manually

        # Campaign Search (for searching WS ID and Mission ID)
        ...
        # Search Results List (display search results, expanded)
        self.search_results_list = QListWidget()
        # self.search_results_list.setMaximumHeight(100) # Remove max height limit
        self.search_results_list.itemClicked.connect(self.on_search_result_clicked)

        # Campaign Name (display selected result)
        ...
        # Campaign ID (WS ID) (display selected result)
        ...
        # Map Name (Mission ID) (display selected result)
        ...
        form_layout.addRow("Search Results:", self.search_results_list)  # Search Results now takes more space
        ...
        # Add form to details_layout
        details_layout.addLayout(form_layout)
        # Delete Legacy Linked State area (no longer needed)

        # Transitions edit area
        ...
        # Transitions text edit (suitable for long commands)
        ...
        main_content_layout.addLayout(left_panel_layout, 1)  # Left side ratio 1
        main_content_layout.addLayout(right_panel_layout, 2)  # Right side ratio 2
    ...
    def load_map_info(self):
        """Load map information data from JSON file"""
        try:
    ...
    def load_fsm_docs(self):
        """Load FSM available conditions and command documentation (optional)"""
        import os
        ...
        # Load from unified function config (can be used as both conditions and actions)
        try:
            from core.FSM.fsm_functions_config import get_functions_doc
            unified_functions = get_functions_doc()
            # Add unified functions to conditions and commands documentation
            ...
            for func_name, func_desc in unified_functions.items():
                ...
                if not existing_cond:
                    self.fsm_docs["conditions"].append({"name": func_name, "desc": f"{func_desc} (unified function, can also be used as action)"})
                # Add to command list
                existing_cmd = [c for c in self.fsm_docs["commands"] if c.get("name") == f"func:{func_name}"]
                if not existing_cmd:
                    self.fsm_docs["commands"].append({"name": f"func:{func_name}", "desc": f"{func_desc} (unified function, can also be used as condition)"})
        except ImportError:
            pass  # Config file doesn't exist, skip

    def show_fsm_docs(self):
        """Show available conditions and commands documentation in popup"""
        ...
        lines = []
        if fields:
            lines.append("Field Descriptions (Transition fields):")
            for f in fields:
        ...
        if cmds:
            lines.append("Available Commands (Actuator Cmds):")
            for c in cmds:
                lines.append(f" - {c.get('name','')}: {c.get('desc','')}")
            lines.append("")

        if examples:
            lines.append("Examples (Transition Examples):")
            for e in examples:
                desc = e.get('desc','')
                val = e.get('value', {})
                try:
            ...
            lines.append("")
        # Use scrollable, resizable dialog to display long text
        dlg = QDialog(self)
        dlg.setWindowTitle("FSM Documentation")
        v = QVBoxLayout(dlg)
        text = QTextEdit(dlg)
    ...
    # Remove Legacy Linked State toggle (deprecated)

    def on_campaign_search_changed(self, text):
        """Update search results list when search box content changes"""
        ...
        for wsid, ws_data in by_wsid.items():
            ...
            for map_obj in maps:
                ...
                if text_lower in map_name.lower():
                    ...
                    # Display map_name, don't show info in parentheses
                    self.search_results_list.addItem(map_name)

    def on_search_result_clicked(self, item):
        """When search result is clicked, fill selected values into corresponding input boxes"""
    ...
    def load_state_machine_from_file(self):
        """Load state machine data from JSON file"""
        ...
        if file_path:
            try:
                ...
                # Convert raw state data to internal format (Linked State removed)
    ...
    def save_state_machine_to_file(self):
        """Save state machine data to JSON file"""
        file_path, _ = QFileDialog.getSaveFileName(self, "Save State Machine", STATE_MACHINE_FILE,
                                                   "JSON Files (*.json)")
        if file_path:
            try:
                # Construct JSON structure to save
                # config default value ["Default Package Name", "Default Author"] if not set
                config_to_save = self.config if self.config else ["Default Package Name", "Default Author"]

                # StateMachine is self.states list
                data_to_save = {
    ...
    def refresh_state_list(self):
        """Refresh left side state list display"""
        self.state_list_widget.clear()
        for state in self.states:
            # Display Key: Campaign Name
    ...
    def add_state(self):
        """Add a new state"""
        # Calculate next Key, based on maximum Key of existing states
        ...
        new_state = {
            "Key": str(next_key),  # Key stored as string
        ...
        self.refresh_state_list()
        # Select newly added state
        self.state_list_widget.setCurrentRow(len(self.states) - 1)
        self.on_state_selected()  # Trigger details update
    ...
    def delete_selected_state(self):
        ...
        if current_row >= 0 and current_row < len(self.states):
            ...
            if reply == QMessageBox.StandardButton.Yes:
                ...
                # Refresh list
                self.refresh_state_list()
                # Clear right side edit area
                self.clear_state_details()

    def on_state_selected(self):
        """Called when left side state list selection changes"""
        current_row = self.state_list_widget.currentRow()
        if current_row >= 0 and current_row < len(self.states):
    ...
    def populate_state_details(self, state):
        """Fill state data into right side edit area"""
        self.key_input.setText(str(state.get("Key", "")))  # Key might be number, convert to string
    ...
    def clear_state_details(self):
        ...
        self.search_results_list.clear()
        self.search_results = []
        # Clear Transitions table and text
        self.transitions_table.setRowCount(0)
        self.transitions_text.clear()

    def save_current_state(self):
        """Save currently edited state on right side"""
        if self.currently_selected_state_index >= 0 and self.currently_selected_state_index < len(self.states):
            ...
            # Update state basic info (Key managed by system, not updated from UI)
            # state["Key"] = self.name_input.text().strip() # Don't update Key
            ...
            # Save Transitions: prioritize parsing text, fall back to table
            ...
            # Refresh list display (if Campaign Name changed)
            self.refresh_state_list()

            # Reselect and load to ensure UI syncs with data
    ...
    def cancel_editing(self):
        """Cancel current editing, restore to last saved state"""
        self.on_state_selected()  # Reload original data of currently selected state

    # Legacy Linked State related functions removed

    # --- Transitions Edit Logic ---
    ...
    # --- Transitions Text Area and Table Sync ---
    ...
    def apply_transitions_text_to_table(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "Invalid Transitions Text", f"JSON parsing failed:\n{e}")
